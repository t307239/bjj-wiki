...
# ── HTML templates (z176→z243: marker bump で 1.0-1.4% drift 解消) ──
# z243 で z176 marker を bump、全 4,698 ページに canonical content 再適用。
# CTA copy / 動作は z176 と完全同一、marker 名のみ z243 に変更。

def bottom_cta_html(c: dict, lang: str) -> str:
    return (
        f'<!-- z243-bottom-cta --><div class="z243-bottom-cta" '
...
def float_cta_html(c: dict, lang: str) -> str:
    ...
    return f'''<!-- z243-float-cta --><div id="z243-float" style="position:fixed;bottom:20px;right:20px;max-width:280px;background:#0d2010;border:1px solid #2e7d32;border-radius:14px;padding:16px 18px;box-shadow:0 4px 20px rgba(0,200,83,.15);z-index:999;display:none;animation:slideUp .3s ease">
<button onclick="document.getElementById('z243-float').style.display='none';try{{localStorage.setItem('z243_float_dismissed',Date.now())}}catch(e){{}}" style="position:absolute;top:8px;right:12px;background:none;border:none;color:#546e7a;font-size:1rem;cursor:pointer;line-height:1" aria-label="Close">✕</button>
...
<script>(function(){{
try{{
  var d=localStorage.getItem('z243_float_dismissed');
  if(d && (Date.now()-parseInt(d,10))<7*86400000) return;
}}catch(e){{}}
var el=document.getElementById('z243-float');
if(!el) return;
var shown=false;
...
# ── z176 rollback regex (z243 で marker bump) ──────────────────────────────
# z176 で 1.0-1.4% drift が累積していたため、z243 marker に bump して全 page
# refresh。z176-bottom-cta / z176-float-cta を完全に削除してから z243 を inject。
Z176_BOTTOM_RE = re.compile(r'<!-- z176-bottom-cta -->.*?</div>\s*\n', re.DOTALL)
Z176_FLOAT_RE = re.compile(r'<!-- z176-float-cta -->.*?\}\)\(\);</script>\s*\n?', re.DOTALL)
# z243: 旧 z224 marker も rollback (drift 92.6%/36.4% 累積で再 bump)
Z224_BOTTOM_RE = re.compile(r'<!-- z224-bottom-cta -->.*?</div>\s*\n', re.DOTALL)
Z224_FLOAT_RE = re.compile(r'<!-- z224-float-cta -->.*?\}\)\(\);</script>\s*\n?', re.DOTALL)
...
def patch_file(fp: Path, lang: str, dry_run: bool) -> tuple[bool, list[str]]:
    ...
    # ─── z243 rollback: remove z175/z176 markers ────────
    # z175 (Top/Bottom/Float, 4-CTA): completely deprecated
    # z176 (Bottom/Float, 2-CTA): superseded by z243 due to content drift accumulation
    ...
    # z243: z176 marker は drift 累積したため bump で全 page refresh
    ...
    # z243: 旧 z224 marker も rollback (drift 累積で再 bump、quality boost 後の検出)
    if "z224-bottom-cta" in c:
        c2 = Z224_BOTTOM_RE.sub("", c)
        if c2 != c:
            c = c2
            actions.append("rm_z224_bottom")
    if "z224-float-cta" in c:
        c2 = Z224_FLOAT_RE.sub("", c)
        if c2 != c:
            c = c2
            actions.append("rm_z224_float")

    # ─── z243 inject: Bottom + Float only (旧 z176 と content 同一) ───
    # 1. Bottom CTA — insert before <footer>
    if "z243-bottom-cta" not in c:
