...
class TestYouTubeButton(unittest.TestCase):
    """
    Content pages generated by the current template should include a YouTube
    search button (.yt-search-btn anchor linking to youtube.com/results).

    Design:
      - Skip index.html (it's a directory page, not a technique article)
      - Sample SAMPLE_SIZE files per language for speed
      - Require ≥ 80% coverage across the sample (older/gear pages pre-date the button;
        actual anchor coverage is ~88 %; 80 % is robust to sampling variance)
      - For pages that DO have the button, validate the href format
    """

    # Actual anchor coverage is ~88 %; threshold is 80 % to remain robust to the
    # deterministic sample hitting gear/older pages that pre-date the button.
    COVERAGE_THRESHOLD = 0.80
    # Match the actual <a> anchor element (not just the CSS class definition).
    YT_ANCHOR_RE = re.compile(r'<a[^>]+yt-search-btn')
    YT_HREF_PATTERN = re.compile(
        r'<a[^>]+yt-search-btn[^>]*href="https://www\.youtube\.com/results\?search_query=[^"]*"'
        r'|'
        r'<a[^>]+href="https://www\.youtube\.com/results\?search_query=[^"]*"[^>]+yt-search-btn'
    )

    def _content_files(self, lang_dir: str) -> list[str]:
        """Return all HTML files except index.html."""
        return sorted(
            f for f in all_html_files(lang_dir)
            if os.path.basename(f) != "index.html"
        )

    def _check_coverage(self, lang: str, lang_dir: str):
        files = sampled(self._content_files(lang_dir))
        # Use the anchor pattern — the CSS class string alone is not enough.
        has_button = [f for f in files if self.YT_ANCHOR_RE.search(read(f))]
        coverage = len(has_button) / len(files) if files else 0
        self.assertGreaterEqual(
            coverage, self.COVERAGE_THRESHOLD,
            f"[{lang}] YouTube ボタン カバレッジ低: "
            f"{len(has_button)}/{len(files)} = {coverage:.1%} "
            f"(閾値 {self.COVERAGE_THRESHOLD:.0%})"
        )

    def _check_href_format(self, lang: str, lang_dir: str):
        """Pages that have the anchor button must point to youtube.com/results?search_query=."""
        files = sampled(self._content_files(lang_dir))
        bad = []
        for path in files:
            content = read(path)
            # Only check pages that actually have the <a> anchor element.
            if not self.YT_ANCHOR_RE.search(content):
                continue
            # The anchor must include a valid youtube.com search href.
            if not re.search(
                r'href="https://www\.youtube\.com/results\?search_query=[^"]+BJJ[^"]*"',
                content,
            ):
                bad.append(os.path.basename(path))
        if bad:
            self.fail(
                f"[{lang}] yt-search-btn の href 形式が不正 ({len(bad)}件): {bad[:5]}"
            )

    def test_en_youtube_button_coverage(self):
        self._check_coverage("en", EN_DIR)

    def test_ja_youtube_button_coverage(self):
        self._check_coverage("ja", JA_DIR)

    def test_pt_youtube_button_coverage(self):
        self._check_coverage("pt", PT_DIR)

    def test_en_youtube_href_format(self):
        self._check_href_format("en", EN_DIR)

    def test_ja_youtube_href_format(self):
        self._check_href_format("ja", JA_DIR)

    def test_pt_youtube_href_format(self):
        self._check_href_format("pt", PT_DIR)
